...
SYNOPSIS: true
 If this setting is enabled, users who have already opted in to Phone Hub will be able to view and download recent photos and videos taken on their phone on Chrome OS.
...
SYNOPSIS: {"https://foo.example": {"run_on_os_login": "allowed"}, "https://bar.example": {"run_on_os_login": "blocked"}, "https://foobar.example": {"run_on_os_login": "run_windowed"}, "*": {"run_on_os_login": "blocked"}}
 This policy allows an admin to specify settings for installed web apps.
       ...
       This policy maps a Web App ID to its specific setting. See https://developer.chrome.com/blog/pwa-manifest-id/ for instructions on how to determine the id for an installed web app.
       A default configuration can be set using the special ID "*", which applies to all web apps without a custom configuration in this policy.
...
SYNOPSIS: false
 This policy allows origin-keyed agent clustering by default.
       ...
       - changes in M101 from site-keyed to origin-keyed. If this policy
