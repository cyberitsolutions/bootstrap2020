...
CREATE TABLE realm (
    name TEXT PRIMARY KEY,
    soe_name TEXT REFERENCES soe(name),
...
CREATE TABLE host (
    name TEXT PRIMARY KEY,
    realm_name TEXT REFERENCES realm(name),
...
CREATE TABLE squid_checksum(
    url TEXT PRIMARY KEY,
    checksum TEXT NOT NULL);

CREATE TABLE squid_rule(
 url               TEXT,
...
CREATE TABLE channel (
    ...
    name TEXT NOT NULL,
    frequency integer NOT NULL REFERENCES station(frequency),
    vpid integer,
    apid integer,
    enabled boolean DEFAULT false NOT NULL);

CREATE TABLE programme (
    sid integer NOT NULL REFERENCES channel(sid),
    channel TEXT,
...
CREATE TABLE local_medium_lifetime (
    lifetime TEXT NOT NULL PRIMARY KEY,
    standard boolean NOT NULL);

CREATE TABLE local_programme (
    address inet NOT NULL REFERENCES local_channel(address),
    play_order integer NOT NULL,
    media_id uuid NOT NULL REFERENCES local_medium(media_id),
...
with (sqlite3.connect(':memory:', detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES) as dst,
    ...
    dst.create_function('regexp', 2, lambda x, y: bool(re.search(x, y)), deterministic=True)

    dst.executescript(schema)

    tca.execute('SELECT * FROM soes')
    dst.executemany('INSERT INTO soe (name) VALUES (:name)', tca)
    tca.execute('SELECT * FROM realms')
    dst.executemany('''INSERT INTO realm (name, soe_name, cidr,
    enabled, staff, boot_curfew, iptv_curfew, web_curfew,
    print_curfew) VALUES (:name, :soe_name, :cidr, :enabled, :staff,
    :boot_curfew, :iptv_curfew, :web_curfew, :print_curfew)''', tca)
    tca.execute('SELECT * FROM hosts')
    dst.executemany('''INSERT INTO host (name, realm_name, mac, ip,
    user_group, enabled, last_uid, last_ping, last_boot) VALUES
    (:name, :realm_name, :mac, :ip, :user_group, :enabled, :last_uid,
    :last_ping, :last_boot)''', tca)

    ppc.execute('SELECT * FROM cups_jobs')
    dst.executemany('''INSERT INTO cups_job (uuid, ts, queue, jobid,
    username, title, copies, options, pages, path) VALUES (:uuid, :ts,
    :queue, :jobid, :username, :title, :copies, json(:options),
    :pages, :path)''', ppc)
    ppc.execute('SELECT * FROM mailstats')
    dst.executemany('INSERT INTO mailstat ( date, pre , held, app , rej) VALUES ( :date, :pre , :held, :app , :rej)', ppc)
    ppc.execute("SELECT * FROM popcon WHERE date > now() - INTERVAL '1 month'")  # don't copy all 500MB
    dst.executemany('''INSERT INTO popcon ( date, hostname, username,
    application, duration) VALUES ( :date, :hostname, :username,
    :application, :duration)''', ppc)
    ppc.execute('SELECT * FROM squid_checksums')
    dst.executemany('INSERT INTO squid_checksum (url, checksum) VALUES (:url, :checksum)', ppc)
    ppc.execute('SELECT * FROM squid_rules')
    dst.executemany('''INSERT INTO squid_rule (url,
    group_restriction, title, policy) VALUES (:url,
    :group_restriction, :title, :policy)''', ppc)

    epg.execute('SELECT * FROM stations')
    dst.executemany('''INSERT INTO station (frequency, name, host,
    card) VALUES (:frequency, :name, :host, :card)''', epg)

    epg.execute('SELECT * FROM channels')
    dst.executemany('''INSERT INTO channel (sid, name, frequency,
    vpid, apid, enabled) VALUES (:sid, :name, :frequency, :vpid,
    :apid, :enabled)''', epg)

    epg.execute('SELECT * FROM programmes')
    dst.executemany('''INSERT INTO programme (sid, channel, start,
    stop, title, sub_title, crid_series, crid_item) VALUES (:sid,
    :channel, :start, :stop, :title, :sub_title, :crid_series,
    :crid_item)''', epg)

    epg.execute('SELECT * FROM channel_curfews')
    dst.executemany('''INSERT INTO channel_curfew (sid,
    curfew_stop_time, curfew_start_time) VALUES (:sid,
    :curfew_stop_time, :curfew_start_time)''', epg)

    epg.execute('SELECT * FROM local_channels')
    dst.executemany('''INSERT INTO local_channel (address, name,
    host) VALUES (:address, :name, :host)''', epg)
    ...
    dst.executemany('''INSERT INTO local_medium (media_id, path, name,
    duration_27mhz, created_at, expires_at) VALUES (:media_id, :path,
    :name, :duration_27mhz, :created_at, :expires_at)''', epg)

    epg.execute('SELECT * FROM local_media_lifetimes')
    dst.executemany('''INSERT INTO local_medium_lifetime (lifetime,
    standard) VALUES (:lifetime, :standard)''', epg)

    epg.execute('SELECT * FROM local_programmes')
    dst.executemany('''INSERT INTO local_programme (address,
    play_order, media_id, last_played) VALUES (:address, :play_order,
    :media_id, :last_played)''', epg)

    epg.execute('SELECT * FROM statuses')
    dst.executemany('''INSERT INTO status (crid_series, status)
    VALUES (:crid_series, :status)''', epg)
