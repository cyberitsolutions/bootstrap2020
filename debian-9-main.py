...
group.add_argument('--ssh-server',
                   default='dropbear',
                   choices=('dropbear', 'openssh-server'),
...
parser.read('debian-9-PrisonPC.site-apps.ini')
...
with tempfile.TemporaryDirectory() as td:
    ...
    subprocess.check_call(
         ...
         *(['--include=libnss-myhostname libnss-resolve',
            ...
            f'--essential-hook=tar-in {create_tarball("debian-9-main")} /',
            '--hook-dir=debian-9-main.hooks',
            ]
           if args.optimize != 'simplicity' else []),
         *(['--include=tzdata',
            '--essential-hook={'
         ...
         *(['--include=nfs-client',  # support NFSv4 (not just NFSv3)
            ...
            f'--essential-hook=tar-in {create_tarball("debian-9-main.netboot")} /']
           if not args.local_boot_only else []),
         *([f'--essential-hook=tar-in {create_tarball("debian-9-main.netboot-only")} /']  # 9% faster 19% smaller
         ...
         *(['--include=mdadm rsnapshot'
            ...
            f'--essential-hook=tar-in {create_tarball("debian-9-datasafe3")} /',
         ...
         *(['--include=smartmontools'
            ...
            f'--essential-hook=tar-in {create_tarball("debian-9-main.disks")} /',
         ...
         *(['--include='
            ...
            *(['--include=firmware-realtek firmware-misc-nonfree']
            ...
            f'--essential-hook=tar-in {create_tarball("debian-9-desktop")} /'
            ]
           if template_wants_GUI else []),
         *(['--include=libnss-ldapd libpam-ldapd unscd',
            f'--essential-hook=tar-in {create_tarball("debian-9-PrisonPC")} /',
            f'--essential-hook=tar-in {create_tarball("debian-9-PrisonPC-staff")} /'
            if args.template.startswith('desktop-staff') else
            f'--essential-hook=tar-in {create_tarball("debian-9-PrisonPC-inmate")} /',
            '--essential-hook={'
         ...
         *([f'--customize-hook=echo bootstrap:{git_description} >$1/etc/debian_chroot',
         ...
         # FIXME: remove once that can/does use rdsquashfs --cat (master server is Debian 9)
         ...
         *(['--dpkgopt=debian-9-PrisonPC/omit-low-level-docs.conf',
            '--hook-dir=debian-9-PrisonPC.hooks']
           if template_wants_PrisonPC else []),
         *(['--hook-dir=debian-9-PrisonPC-inmate.hooks']
         ...
         *(['deb [check-valid-until=no] http://snapshot.debian.org/archive/debian/20210410/ stretch main']
           if args.template == 'datasafe3' else []),
         *([f'deb [signed-by={pathlib.Path.cwd()}/debian-9-PrisonPC.packages/PrisonPC-archive-pubkey.asc] https://apt.cyber.com.au/PrisonPC stretch desktop']  # noqa: E501
           if template_wants_PrisonPC else []),
         ])
...
if args.boot_test:
    ...
    with tempfile.TemporaryDirectory(dir=destdir) as testdir:
        ...
        if template_wants_PrisonPC:
            ...
            (testdir / 'site.dir/etc/nftables.conf.d/9-PrisonPC-master-server-address.conf').write_text(
                f'define PrisonPC = {master_address};')
            (testdir / 'site.dir/etc/nftables.conf.d/90-boot-test.conf').write_text(
                pathlib.Path('debian-9-PrisonPC/firewall-boot-test.nft').read_text())
        subprocess.check_call([
