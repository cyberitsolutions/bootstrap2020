...
with dst_path.open('w') as f:
    ...
    if m := [d for d in debnames if good_pattern.fullmatch(d)]:
        print('\n\nPin: version *\nPin-Priority: 500\nPackage:', *m, file=f)
    if m := [f'src:{d}' for d in dscnames if good_pattern.fullmatch(d)]:
        print('\n\nPin: version *\nPin-Priority: 500\nPackage:', *m, file=f)
    if m := [d for d in debnames if shit_pattern.match(d)]:
        print('\n\nPin: version *\nPin-Priority: -13646\nPackage:', *m, file=f)
    if m := [f'src:{d}' for d in dscnames if shit_pattern.match(d)]:
        print('\n\nPin: version *\nPin-Priority: -18418\nPackage:', *m, file=f)
