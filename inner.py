...
subprocess.check_call([
    ...
    '--key=/tmp/key',
    '--cert=/tmp/cert',
    '--output=/linuxx64.efi',   # <-------- IMPORTANT NOTE YOU MUST COPY THIS INTO /srv/netboot/images MANUALLY, BECAUSE THIS SCRIPT LACKS PERMISSIONS TO DO SO
    '/linuxx64.unsigned.efi'])
...
#         cwd=td)
# Actually boot it.
# pathlib.Path('/FUCK.IPXE').write_text('#!ipxe\nboot --replace vmlinuz.efi\n')
...
subprocess.check_call([
    ...
    '-netdev', 'id=OutclassMountingBoggle,type=user,tftp=/,bootfile=FUCK.IPXE,dnssearch=lan',
    '-machine', 'q35,smm=on',
